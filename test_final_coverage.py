# Add specific mock tests for the big missing functions
def add_final_tests():
    with open('tests/test_coverage_ext8.py', 'w') as f:
        f.write('''
import pytest
...
def test_backup_encrypt(tmp_path, monkeypatch):
    import datetime
    ...
    # mock gpg
    monkeypatch.setenv("AGM_BACKUP_PASSWORD", "test")
    import subprocess
    monkeypatch.setattr(subprocess, "run", lambda *a, **k: type('C', (), {'returncode': 0})())

    args = argparse.Namespace(source_dir=str(tmp_path), gemini_home=str(tmp_path), backup_dir=str(tmp_path), dry_run=False, force=True, encrypt=True, auth_only=False, include_bin=False, include_logs=False)
    perform_backup(args)
