#!/usr/bin/env python3
# src/gemini_manager/integrity.py
...
from .protected import get_diff_excludes
import subprocess
import shlex
...
def perform_integrity_check(args: argparse.Namespace):
    ...
    diff_proc = run(f"diff -r {diff_excludes} {shlex_quote(src)} {shlex_quote(latest_backup)}", check=False, capture=True)
