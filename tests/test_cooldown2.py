...
from antigravity_manager.cooldown import evaluate_metadata, evaluate_entries, format_model_usage, ModelCooldown
    ...
    m = {
        "status": {
            "models": [
                ...
                {"model_name": "gemini 3.5 flash", "is_available": False, "refresh_at": "2024-01-01T12:00:00+00:00"}
            ]
        },
        "email": "user"
    }
    st = evaluate_metadata(m, source="test", now=datetime(2024, 1, 1, 10, tzinfo=timezone.utc))
    assert st.email == "user"
    assert st.available_models == 1
    assert st.total_models == 2

def test_evaluate_entries(monkeypatch):
    e1 = BackupEntry(Path("a"), "user", "pro", "now", "now", "now", "mode", {"email": "user", "status": {"models": []}})
    e2 = BackupEntry(Path("b"), "other", "pro", "now", "now", "now", "mode", {"email": "other", "status": {"models": []}})
