...
from antigravity_manager.ui import banner, print_rich_help, custom_theme
...
from antigravity_manager.status import live_status_to_text, LiveStatus, ModelQuotaStatus
from antigravity_manager.list_backups import print_entries_table, BackupEntry
from antigravity_manager.cooldown import print_statuses_table, CooldownStatus
...
def test_backup_result_to_text():
    panel = backup_result_to_text(Path("/archive"), Path("/meta"), {"email": "test@test.com"}, dry_run=False)
    assert panel is not None

    panel = backup_result_to_text(Path("/archive"), Path("/meta"), {"email": "test@test.com"}, dry_run=True)
    assert panel is not None

def test_restore_result_to_text():
    panel = restore_result_to_text(Path("/archive"), {}, [Path("/file1")], Path("/safety"), dry_run=False, full=True)
    assert panel is not None
...
def test_remove_result_to_text():
    panel = remove_result_to_text({"local_files_removed": ["/path"], "local_registry_removed": True}, email="a@b.c", dry_run=False)
    assert panel is not None
...
def test_live_status_to_text():
    model1 = ModelQuotaStatus(model_name="M1", quota_percent_left=100, is_available=True, refresh_in_text=None, refresh_at=None)
    model2 = ModelQuotaStatus(model_name="M2", quota_percent_left=30, is_available=True, refresh_in_text=None, refresh_at=None)
    model3 = ModelQuotaStatus(model_name="M3", quota_percent_left=5, is_available=True, refresh_in_text=None, refresh_at=None)
    model4 = ModelQuotaStatus(model_name="M4", quota_percent_left=0, is_available=False, refresh_in_text="1h", refresh_at=datetime.now())
    model5 = ModelQuotaStatus(model_name="M5", quota_percent_left=None, is_available=True, refresh_in_text=None, refresh_at=None)

    status = LiveStatus(
        email="t@t.com", plan="Pro", is_pro=True, captured_at=datetime.now(),
...
def test_print_statuses_table(capsys, monkeypatch):
    ...
    m = ModelCooldown(name="m", quota_percent_left=100, is_available=True, refresh_at=None, remaining_seconds=0)
