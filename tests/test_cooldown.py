...
from antigravity_manager.cooldown import evaluate_model, find_decision_model, ModelCooldown, format_remaining, read_active_email
import json
...
def test_evaluate_model():
    now = datetime.now()
    m = evaluate_model({"model_name": "x", "quota_percent_left": 100, "is_available": True, "refresh_at": str(now + timedelta(seconds=10))}, now=now)
    assert m.name == "x"
