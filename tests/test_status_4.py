from antigravity_manager.status import capture_tmux_status_text, wait_for_prompt, wait_for_usage_panel
import pytest
...
def test_capture_tmux_status_text(monkeypatch):
    ...
    monkeypatch.setattr("antigravity_manager.status.wait_for_usage_panel", lambda *a, **kw: "Mock Panel Text")
...
def test_wait_for_prompt(monkeypatch):
    ...
    monkeypatch.setattr("antigravity_manager.status.capture_pane", lambda x: "user@example.com (pro)\n> ")
    wait_for_prompt("pane", timeout_seconds=1)

def test_wait_for_usage_panel(monkeypatch):
    import time
    monkeypatch.setattr(time, "sleep", lambda x: None)
    monkeypatch.setattr("antigravity_manager.status.capture_pane", lambda x: "Model Quota\nuser@example.com (pro)\n(Gemini 3.5 Flash)\n  Quota: 50%\n")
